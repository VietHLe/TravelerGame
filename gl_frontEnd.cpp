...
const int GRID_PANE_HEIGHT = 900;
const int STATE_PANE_WIDTH = 300;
const int STATE_PANE_HEIGHT = 900;
...
void drawTraveler(const Traveler& traveler)
{
	...
	if (traveler.segmentList.size() > 1)
	{
		...
		glBegin(GL_LINES);
			glVertex2f(0, 0);
			glVertex2f(segMove[traveler.segmentList[traveler.segmentList.size()-1].dir][0]*0.65f,
					   segMove[traveler.segmentList[traveler.segmentList.size()-1].dir][1]*0.65f);
		glEnd();
	}
	else
	{
...
void drawMessages(int numMessages, char** message)
{
	...
	{
		displayTextualInfo(message[k], LEFT_MARGIN, 3*STATE_PANE_HEIGHT/4 - k*V_PAD, LARGE_FONT);
	}

	//	display info about number of live threads
	char infoStr[256];
	sprintf(infoStr, "Live Threads: %d", numLiveThreads);
	displayTextualInfo(infoStr, LEFT_MARGIN, 7*STATE_PANE_HEIGHT/8, LARGE_FONT);
}
